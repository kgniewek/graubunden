...
export default function MobileSettingsModal({
    ...
    const getActiveStyle = (styleId: string) => selectedMapStyle === styleId;
    ...
    return (
        ...
        >
            ...
            >
                {/* Header */}
                <div className="flex items-center justify-between pt-4 pl-5 pr-4 pb-4">
                    <h2 className="font-bold text-2xl">
                        <T en="Preferences" de="Einstellungen" it="Preferenze" fr="Préférences" />
                    </h2>
                    <button
                ...
                <div className="space-y-5 pb-5 pl-5 pr-5">
                    <h3 className="font-bold text-lg">
                        <T en="Location Filters" de="Standortfilter" it="Filtri località" fr="Filtres de localisation" />
                    </h3>
              ...
              <span className="text-md font-medium">
                <T en="Show only Editor's Choice locations" de="Nur Editor's Choice Standorte anzeigen" it="Mostra solo le località scelte dall'editore" fr="Afficher seulement les choix de l'éditeur" />
              </span>
                        ...
                        <div className="flex items-center justify-between">
              <span className="text-md font-medium">
                <T en="Show locations only within Switzerland" de="Nur Standorte innerhalb der Schweiz anzeigen" it="Mostra solo località in Svizzera" fr="Afficher uniquement les lieux en Suisse" />
              </span>
                        ...
                        <div className="flex items-center justify-between">
              <span className="text-md font-medium">
                <T en="Show locations only within Graubünden" de="Nur Standorte innerhalb Graubündens anzeigen" it="Mostra solo località nei Grigioni" fr="Afficher uniquement les lieux dans les Grisons" />
              </span>
                ...
                <div className="space-y-3 p-5">
                    <h3 className="font-bold text-lg">
                        <T en="Map Source" de="Kartenquelle" it="Fonte della mappa" fr="Source de la carte" />
                    </h3>
                        ...
                        {mapStyles.map((style) => {
                            ...
                            return (
                                <button
                                    key={style.id}
                                    onClick={() => {
                                        if (style.id === 'light-simple') {
                                            onMapThemeChange('light');
                                            onMapStyleChange('light-simple');
                                        } else if (style.id === 'dark-simple') {
                                            onMapThemeChange('dark');
                                            onMapStyleChange('dark-simple');
                                        } else {
                                            onMapStyleChange(style.id);
                                        }
                                    }}

                                    className={`relative aspect-[3/2] rounded-md border-2 overflow-hidden ${isActive ? 'border-primary' : 'border-border'}`}
                                >
                                    <div className="absolute top-0 left-0 right-0 bottom-[20px]">
                                        <img src={style.image} alt={style.name[language]} className="w-full h-full object-cover object-top" />
                                    ...
                                    <div className={`absolute bottom-0 left-0 right-0 px-1 py-[3px] z-10 text-xs font-medium text-center truncate leading-tight
                    ${isActive ? 'bg-black text-white dark:bg-white dark:text-black' : 'bg-white/80 text-black dark:bg-black dark:text-white'}`}>
                                        {style.name[language]}
                                    </div>
                                </button>
                            );
